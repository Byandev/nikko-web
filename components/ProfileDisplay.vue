...
const bannerUrl = computed(() => props.freelancerDetails?.user?.banner?.original_url ?? defaultBannerUrl);
const avatarUrl = computed(() => props.freelancerDetails?.user?.avatar?.original_url ?? defaultAvatarUrl);

const joinedDate = computed(() =>
    props.freelancerDetails?.user?.created_at
        ? new Date(props.freelancerDetails.user.created_at).toLocaleString('en-US', {
            month: 'long',
            day: 'numeric',
            year: 'numeric',
        })
...
<template>
    <div v-if="freelancerDetails"
        class="text-center relative border border-gray-300 rounded-lg overflow-hidden bg-white p-4">
        ...
        <div class="mt-20 sm:mt-4 mb-4 px-4 flex flex-col sm:flex-row items-center sm:items-start">
            <div class="sm:ml-44 mt-4 sm:mt-0 text-center sm:text-left">
                <h2 class="text-2xl font-bold text-gray-900">{{ freelancerDetails?.user?.first_name }} {{
                    freelancerDetails?.user?.last_name }}</h2>
                <p class="mt-2 text-md text-gray-600">Joined on {{ joinedDate }}</p>
                <div class="mt-2 flex items-center space-x-2 justify-center sm:justify-start">
                    <Icon icon="mdi:map-marker" width="15" height="15" />
                    <span class="text-sm text-black font-semibold">{{ freelancerDetails?.user?.country_code }}</span>
                </div>
            </div>
        </div>
    </div>
