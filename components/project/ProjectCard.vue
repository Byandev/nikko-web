...
import { ExperienceLevelToText, type Project, ProjectLengthToText, Term, Level } from "~/types/models/Project";
...
const props = defineProps<{ project: Project; showSaveButton?: boolean, showWithdrawApplication?: boolean, showApplyButton?: boolean, viewAs: AccountType, showDropDownButton?:boolean }>();
...
const handleDelete = () => {
  if (props?.project.id !== undefined) {
    emit('withdraw-proposal', props.project.id);
  }
};
...
const form = ref<FormData>(props.project ? {
  title: props.project.title || '',
  description: props.project.description || '',
  length: props.project.length || Term.LONG_TERM,
  experience_level: props.project.experience_level || '',
  languages: props.project.languages ? props.project.languages.map((lang: { name: string }) => ({ name: lang.name })) : [],
  estimated_budget: Number(props.project.estimated_budget) || 0,
...
const submitForm = async () => {
  ...
  try {
    ...
    await updateJob(`/v1/client/projects/${props.project.id}`, {
      method: 'PUT',
  ...
  <div
    ...
    <div class="w-8/12 px-5 py-5 space-y-4">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between">
        ...
        <div class="flex items-center gap-2" v-if="props.showDropDownButton">
  ...
  <Modal :modelValue="isEditModalOpen" @update:modelValue="isEditModalOpen = $event">
    ...
    <template #content>
      <form @submit.prevent="submitForm" class="w-full max-w-lg text-left">
            ...
            <div v-else>
              <div>
                <select @change="onSelectedLanguage" v-model="selectedLanguageName"
                  ...
                  <option :value="language" class="truncate text-sm leading-6" v-for="language in languageOptions"
                    :key="language">
                    {{ language }}
                  </option>
                </select>
              ...
              <ul class="mt-2 space-y-4">
                <li v-for="(image, index) in formRef.images" :key="index"
                  ...
                  <div class="flex items-center space-x-2 w-full">
                    ...
                    <span class="text-sm text-gray-900 truncate max-w-52 lg:max-w-96" :title="image.name">{{ image.name
                      }}</span>
                  </div>
                </li>
              </ul>
          ...
          <div class="mt-2">
            ...
            <div v-else>
              <select id="skills" required @change="onSelectSkill" v-model="selectedSkillId"
                ...
                <option class="truncate text-sm leading-6 whitespace-nowrap" v-for="skill in skillOptions"
        ...
        <div class="flex mt-5 w-full flex-wrap" :class="project.status == 'DRAFT' ? 'justify-between' : 'justify-end'">
          <Button text="Cancel" type="button" background="white" foreground="primary" @click="{
            isEditModalOpen = false;
            resetJobPosting();
          }" />
          <div class="flex flex-row flex-wrap">
            <Button v-if="project.status == 'DRAFT'" text="Save as Publish" type="submit" background="white"
              foreground="primary" @click="{
                isEditModalOpen = false;
                status = 'ACTIVE'
              }" />
            <Button :is-loading="isSubmitting || isUploading" text="Publish" type="submit" background="primary"
              foreground="white" />
          </div>
        </div>
      </form>
