  ...
  <nav aria-label="Progress" class="w-full">
    <ol role="list" class="flex flex-wrap items-center justify-center">
      <li v-for="(step, index) in props.steps" :key="index" :class="{'relative': true, 'pr-4 sm:pr-8 md:pr-12 lg:pr-16 xl:pr-20': !stepClasses[index].isLast}">
        ...
        <a href="#" :class="`relative flex h-8 w-8 items-center justify-center rounded-full ${stepClasses[index].stepClass}`" :aria-current="step.state === 'current' ? 'step' : undefined">
          ...
          <template v-else>
            <span :class="step.state === 'upcoming' ? 'text-primary' : 'text-white'">{{ index + 1 }}</span>
          </template>
