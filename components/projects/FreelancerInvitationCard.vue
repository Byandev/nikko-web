...
const hasLongDescription = computed(() => props.proposal.cover_letter.length > 300);
        ...
        <div class="w-9/12 px-5 py-5 space-y-4">
            <div class="flex flex-row justify-between gap-4">
                <div class="flex flex-row gap-3 items-center">
                    <!-- <img :src="props.proposal.account.user?.avatar?.original_url || defaultAvatarUrl" alt="profile"
                        class="w-16 h-16 rounded-full">
                    <div class="flex flex-col justify-center">
                        <div class="flex justify-between">
                            <span class="text-lg font-bold" v-if="props.proposal.account.user">
                                {{ props.proposal.account.user.first_name || 'No name provided' }} {{
                                props.proposal.account.user.last_name || '' }}
                            </span>
                        </div>
                        <span class="text-sm text-gray-500" v-if="props.proposal.account.user">{{
                ...
                <div>
                    ...
                    <HeartIcon v-if="showSaveButton" @click="toggleSave"
                        ...
                        :class="props.proposal ? 'fill-primary' : ''" />
                </div>
            </div>
            <!-- <div v-if="props.proposal.account.skills && props.proposal.account.skills.length > 0"
                class="mt-4 w-full">
                ...
                <div class="mt-2 flex flex-row gap-2 flex-wrap">
                    <div v-for="skill in props.proposal.account.skills" :key="`selected-skill-${skill.id}`">
                        <span
            ...
            <p class="">
                {{ hasLongDescription && showAllDescription ? proposal.cover_letter : proposal.cover_letter.slice(0, 300) }}
            </p>
