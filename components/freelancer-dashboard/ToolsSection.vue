      ...
      <div class="mt-5 flex flex-wrap">
        <div v-for="(tool, idx) in account?.tools" :key="`selected-tool-${tool.id}`" class="mr-2 my-1">
          <span
            class="inline-flex items-center gap-x-0.5 rounded-md bg-green-50 px-2 py-1 text-sm font-medium text-green-700 ring-1 ring-inset ring-green-600/20">
            {{tool.name}}
            <button @click="onRemoveTool(idx)" type="button"
            ...
            <div class="relative mt-1">
              <ListboxButton
                ...
                <span class="block truncate">
                  <span v-if="selectedTools.length === 0">Select Skill</span>
                  <span v-else>
                    <span v-for="(tool, index) in selectedTools" :key="index">
                      {{ tool.name }}<span v-if="index < selectedTools.length - 1">, </span>
                    </span>
                  </span>
                </span>
              ...
              <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                ...
                <ListboxOptions
                  ...
                  <ListboxOption v-for="(tool, index) in toolOptions?.data" v-slot="{ active, selected }" :key="index"
                    :value="tool" as="template">
                    <li
                      :class="[active ? 'bg-primary/10 text-primary' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-10 pr-4']">
                      <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">{{ tool.name
                        }}</span>
          ...
          <div class="flex flex-wrap mt-2">
            <div v-for="(tool, idx) in selectedTools" :key="`selected-tool-${tool.id}`" class="mr-2 my-1">
              <span
                class="inline-flex items-center gap-x-0.5 rounded-md bg-green-50 px-2 py-1 text-sm font-medium text-green-700 ring-1 ring-inset ring-green-600/20">
                {{tool.name}}
                <button @click="onRemoveSelectedTool(idx)" type="button"
                  class="group relative -mr-1 h-3.5 w-3.5 rounded-sm hover:bg-green-600/20">
                  <span class="sr-only">Remove</span>
                  <svg viewBox="0 0 14 14" class="h-3.5 w-3.5 stroke-green-700/50 group-hover:stroke-green-700/75">
                    <path d="M4 4l6 6m0-6l-6 6" />
