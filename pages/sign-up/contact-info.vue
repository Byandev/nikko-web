    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="flex flex-col sm:flex-row gap-5 mt-4">
        <div class="w-full sm:w-1/2">
          ...
          <div class="mt-2">
            <select required @change="form.state_code = ''" v-model="form.country_code"
        ...
        <div class="w-full sm:w-1/2">
          ...
          <div class="mt-2">
            <select @change="form.city = ''" v-model="form.state_code"
                    class="w-full px-2 block text-sm leading-6 rounded-md border-0 py-2 shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600">
              <option :value="state.isoCode" class="truncate text-sm leading-6 " v-for="state in stateOptions" :key="state.isoCode">
                {{ state.name }}
              </option>
            </select>
          </div>
